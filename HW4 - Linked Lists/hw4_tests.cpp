...
CPSC 223-01
HW4
This hw4_tests.cpp file tests functions 
...
ASSERT_LE(sorted_ks[ i ], sorted_ks[i +1]);//changed
}

/*Test 7 - checks that insert, remove, and 
size works with multiple cases.*/
...
c.insert("a", 10.0);
c.insert("b", 20.0);
LinkedListCollection <string, double> c_copy = c;
LinkedListCollection <string, double> c_copy_two = c_copy;
ASSERT_EQ(c.size(), c_copy.size());
ASSERT_EQ(c_copy.size(), c_copy_two.size());
} 

/* Test 10 - checks that the assignment 
operator function works.
*/
TEST(BasicCollectionTest, AssignmentOperator) {
LinkedListCollection <string, double> c;
LinkedListCollection <string, double> c_copy;
c.insert("a", 10.0);
c.insert("b", 20.0);
c_copy.insert ("c", 30.0);
c_copy.insert ("d", 40.0);
c_copy.insert ("e", 50.0);
c = c_copy;
ASSERT_EQ(c.size(), c_copy.size());
}

/* Test 11 - checks that the destructor 
works
*/
TEST(BasicCollectionTest, Destructor) {
LinkedListCollection <string, double> c;
c.insert("a", 10.0);
c.insert("b", 20.0);
c.~LinkedListCollection();
ASSERT_EQ(c.size(), 0);
}
